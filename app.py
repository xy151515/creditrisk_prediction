...
def load_model_scaler_and_metrics():
    try:
        ...
        st.error("Failed to load the model or other files. Please ensure they are correctly saved and accessible.")
        raise
...
if menu == "Home":
    ...
    try:
        ...
        st.error("Training data file not found. Please ensure 'model_train.csv' is available.")

elif menu == "Evaluation Metrics":
    st.subheader("Model Evaluation Metrics")
    evaluation_metrics = pd.DataFrame({
        'Metric': ['Accuracy', 'Precision', 'Recall', 'F1-Score'],
        'Score': [metrics.get('Accuracy', 0), metrics.get('Precision', 0), metrics.get('Recall', 0), metrics.get('F1-Score', 0)]
    })
    ...
    try:
        ...
        st.error("Model does not support feature importance or it failed to load properly.")
